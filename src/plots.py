...
def plot_revenue_by_month_year(df: DataFrame, year: int):
    ...
    sns.lineplot(x="month", y=f"Year{year}", data=df, marker="o", sort=False, ax=ax1)
    ax2 = ax1.twinx()
...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    # Adding tags and title to the graphic
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    """
    # Convert the column 'date' from timestamp to datetime format
    df['date'] = pd.to_datetime(df['date'], unit='ms')

    # Creating a figure and axis
    plt.figure(figsize=(12, 6))

    # Chart of orders per day
    plt.plot(df['date'], df['order_count'], label='Cantidad de órdenes por día', color='blue')
    ...
    plt.title('Cantidad de órdenes por día en 2017 con días feriados destacados')
    plt.legend()
    plt.grid(True)

    # Automatic adjustment of date format on the x-axis
